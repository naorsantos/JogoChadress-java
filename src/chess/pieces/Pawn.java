	...
	public boolean[][] possibleMoves() {
		...
		if (getColor() == Color.WHITE) {
			...
			if (position.getRow() == 3) {
				...
				if (getBoard().positionExists(esquerda) && isThereOpponentPiece(esquerda)
						&& getBoard().piece(esquerda) == chessMatch.getEnPassantVunerable()) {
					mat[esquerda.getRow() - 1][esquerda.getColumn()] = true;
				}
				Position direita = new Position(position.getRow(), position.getColumn() + 1);
				if (getBoard().positionExists(direita) && isThereOpponentPiece(direita)
						&& getBoard().piece(direita) == chessMatch.getEnPassantVunerable()) {
